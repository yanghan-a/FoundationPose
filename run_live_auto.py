...
姿态输出: 直接使用 FoundationPose 网络输出, 不做任何后处理修正。
  立方体几何对称, 网络收敛到的"+X 朝向哪个面"由 mesh OBJ 文件 frame 决定。
  可视化箭头: X=红色, Y=蓝色, Z=白色 (颜色仅用于区分轴, 不代表物理面颜色)。
...
if __name__ == '__main__':
    ...
    parser.add_argument('--zmq_port', type=int, default=6666,
                        help='ZMQ PUB port for cube pose (default 6666, 0=disable)')
    ...
    parser.add_argument('--box_w', type=float, default=0.6,
                        help='固定框宽度占图像宽度比例 (0~1)')
    parser.add_argument('--box_h', type=float, default=0.6,
        ...
        while True:
            ...
            if need_register:
                ...
                if not reg_valid:
                    ...
                    last_good_pose = None  # 强制 fallback 到 FixedBoxSelector 重试
                    continue

                # 直接使用网络输出, est.pose_last 已由 register 内部置为 centered pose
                need_register = False
                frame_idx = 0
                lost_count = 0
                fps_counter = FPSCounter()
            ...
            if zmq_socket is not None:
                ...
                if world_calibrator is not None and world_calibrator.is_fixed:
                    R_world, t_world = world_calibrator.cam_to_world(pose)
                    world_fixed = True
            ...
            if debug >= 1:
                center_pose = pose @ inv_to_origin
                vis_bgr = cv2.cvtColor(color, cv2.COLOR_RGB2BGR)
                draw_posed_3d_box(cam.K, img=vis_bgr, ob_in_cam=center_pose, bbox=bbox)
                axis_pose = pose.copy()
                ...
                if world_calibrator is not None and world_calibrator.is_fixed:
                    ...
                    pos = pose[:3, 3]
                    euler_deg = Rotation.from_matrix(pose[:3, :3]).as_euler('xyz', degrees=True)
                    frame_label = "cam"
